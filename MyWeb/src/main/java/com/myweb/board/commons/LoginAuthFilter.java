...
public class LoginAuthFilter extends HttpFilter implements Filter {
	...
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain) throws IOException, ServletException {
		...
		//평소 사용하는 타입은 HttpServletRequest이다.
		//부모 타입의 ServletRequest를 자식 타입으로 끌어내려 사용한다.
		HttpSession session = ((HttpServletRequest) request).getSession();
									//자식
		if(session.getAttribute("user") == null) {
			//로그인 안함/로그아웃/세션만료
			System.out.println("회원 권한 없음 통과 컷");
		...
		//통과한 요청은 서블릿 클래스로 진입한다.
		chain.doFilter(request, response);
	}

	public void init(FilterConfig fConfig) throws ServletException {
		//웹 컨테이너(서버)가 시작될 때 필터 객체를 생성한다.
		//이 때 객체가 생성되면서 최초 한 번 호출되는 메서드이다.
		//필터에서 처리 시 필요한 객체 등을 초기화(JDBC 관련 설정)할 때 사용한다.
